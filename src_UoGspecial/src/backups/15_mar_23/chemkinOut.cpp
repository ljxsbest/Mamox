...
{
	XML xmlFile("resources/namesList/molList.xml", 'r');
...
void ChemkinOut::completeDetailed()
{
	...
	for (int i = 0; i < detReacSpecies.size(); i++)
	{
		...
		outFileDetailed << std::left << setw(20) << detReacSpecies[i][0] << " => " << setw(30) << detReacSpecies[i][1] << "    " << detReacConst[i] << endl;	// print the reaction
