...
std::string ThermoOut::NASAOutput(Molecola* mol, std::string name)
{
	...
	else
	{
		...
		fitNASAParam(Temps, CpsVec, Hf, 298.0, S, 298.0, &paramLowT);
		paramHighT = paramLowT;
...
std::string ThermoOut::NASAParametersToString(std::string name, int numC, int numH, int numO, std::vector<double> Tlims, Eigen::VectorXd paramLowT, Eigen::VectorXd paramHighT)
{
	...
	std::sprintf(buffer, "%+.8E%+.8E%+.8E%+.8E%+.8E    2", paramHighT(0), paramHighT(1), paramHighT(2), paramHighT(3), paramHighT(4));
	output << buffer << std::endl;
	std::sprintf(buffer, "%+.8E%+.8E%+.8E%+.8E%+.8E    3", paramHighT(5), paramHighT(6), paramLowT(0), paramLowT(1), paramLowT(2));
	output << buffer << std::endl;
	std::sprintf(buffer, "%+.8E%+.8E%+.8E%+.8E                   4", paramLowT(3), paramLowT(4), paramLowT(5), paramLowT(6));
	output << buffer << std::endl;
